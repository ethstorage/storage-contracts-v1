...
class Poseidon:
    def __init__(self, p, security_level, alpha, input_rate, t, full_round: Optional[int] = None,
        ...
        if rc_list is not None:
            ...
            self.rc_field = self.field_p([int(x, 16) if isinstance(x, str) else x for x in rc_list])
        else:
    ...
    def run_hash_state(self, input_vec: list):
        """
...
""" Fast Poseidon without using galois library """
class FastPoseidon:
    def __init__(self, p, security_level, alpha, input_rate, t, full_round: Optional[int] = None,
                 partial_round: Optional[int] = None, mds_matrix: Optional[list] = None,
                 rc_list: Optional[list] = None, prime_bit_len: Optional[int] = None):
        ...
        print("Initialize field")
        self.field_p = lambda x: x

        print("Initialize MDS matrix")
        if mds_matrix is not None:
            if (len(mds_matrix) != self.t) & (len(mds_matrix[0]) != self.t):
                raise ValueError('Invalid size of MDS matrix')
            self.mds_matrix = self.get_field_matrix_from_hex_matrix(mds_matrix)
        else:
            raise ValueError("mds not found")
    ...
    def get_field_matrix_from_hex_matrix(self, mds_matrix):
        """
        ...
        mds_matrix_field = []
        for i in range(0, n):
            mds_matrix_field.append([0] * n)
            for j in range(0, n):
                mds_matrix_field[i][j] = int(mds_matrix[i][j], 16)
        return mds_matrix_field

    def s_box(self, element):
        return pow(element, self.alpha, self.p)
    
    def mds_mul(self, state, mds_matrix):
        return [sum([state[j] * mds_matrix[j][i] for j in range(len(state))]) % self.p for i in range(len(state))]

    def full_rounds(self):
        for r in range(0, self.half_full_round):
            # add round constants, apply s-box
            for i in range(0, self.t):
                self.state[i] = (self.state[i] + self.rc_field[self.rc_counter]) % self.p
                self.rc_counter += 1

                self.state[i] = self.s_box(self.state[i])

            # apply MDS matrix
            self.state = self.mds_mul(self.state, self.mds_matrix)
    ...
    def run_hash(self, input_vec: list):
        return self.run_hash_state(input_vec)[1]
