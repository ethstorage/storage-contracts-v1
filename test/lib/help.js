...
async function execAllSamples(StorageContract, randomChecks, blockNumber, miner, nonce, startShardId) {
  ...
  for (let i = 0; i < randomChecks; i++) {
    let [encodingkey, sampleKvIdx, sampleIdxInKv, decodedSample, encodedSample] = await getSampleIdxByHash(
      StorageContract,
      startShardId,
      hash0,
      miner
...
async function getIntegrityProof(
  decodeProof,
  Mask,
  encodingKey,
  sampleKvIdx,
  ...
  let [root, merkleProof] = await getMerkleProof(
    sampleKvIdx,
    sampleIdxInKv,
    decodedSampleData,
    StorageContract,
  ...
  let integrityProof = await getSampleProof(
    root,
    merkleProof,
    decodeProof,
    encodingKey,
