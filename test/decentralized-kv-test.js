...
describe("DecentralizedKV Test", function () {
  ...
  it("removes", async function () {
    ...
    // write random data
    for (let i = 0; i < 10; i++) {
      await kv.connect(addr0).put(ethers.encodeBytes32String(i.toString()), ethers.toBeHex(i));
    }

    for (let i = 0; i < 5; i++) {
      await kv.connect(addr1).put(ethers.encodeBytes32String(i.toString()), ethers.toBeHex(i + 100));
    }
