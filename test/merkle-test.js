...
describe("MerkleLib Test", function () {
  ...
  it("full zero data verify", async function () {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      let chunkData = data.subarray(i * 64, (i + 1) * 64);
  ...
  it("8K data testing", async function () {
    ...
    expect(await ml.verify(d.subarray(0, 4096), 0, root, [...leftSliceProof])).to.be.true;
    const rightSliceProof = await ml.getProof(d, 4096, 1, 1);
    expect(await ml.verify(d.subarray(4096, 8192), 1, root, [...rightSliceProof])).to.be.true;
  });
  ...
  it("should forbid illegal parameter", async function () {
    ...
    expect(await ml.verify(data.subarray(0, 64), 0, root, [...proof])).to.equal(true);
    // out of bound index
    await expect(ml.verify(data.subarray(0, 64), 8, root, [...proof])).to.be.revertedWith("chunkId overflows");
    await expect(ml.verify(data.subarray(0, 64), 10, root, [...proof])).to.be.revertedWith("chunkId overflows");
  });
