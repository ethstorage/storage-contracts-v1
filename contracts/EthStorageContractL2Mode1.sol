...
/// @custom:proxied
/// @title EthStorageContractL2Mode1
/// @notice EthStorage contract that will be deployed on L2, and uses L1Block contract to mine.
contract EthStorageContractL2Mode1 is EthStorageContract1 {
    ...
    /// @notice Constructs the EthStorageContractL2Mode1 contract.
    constructor(
    ...
    function _checkAppend(uint256 _batchSize) internal virtual override {
        ...
        require(msg.value >= totalPayment - sgtCharged, "EthStorageContractL2Mode1: not enough batch payment");
        ...
        returns (bytes32)
    {
        ...
        require(bh != bytes32(0), "EthStorageContractL2Mode1: failed to obtain blockhash");
    ...
    function _checkUpdateLimit(uint256 _updateSize) internal override {
        ...
        require(blobsUpdated + _updateSize <= UPDATE_LIMIT, "EthStorageContractL2Mode1: exceeds update rate limit");
