...
echo "===== Starting $CONTRACT_NAME Prepare ====="
...
if ! grep -q "ONCHAIN EXECUTION COMPLETE & SUCCESSFUL." "$LOG_FILE"; then
  echo "Error: Prepare failed. Check the log file: $LOG_FILE"
  exit 1
fi
...
if [ -z "$NEW_IMPL_ADDRESS" ]; then
  echo "Error: Prepared implementation address not found. Check $LOG_FILE"
  exit 1
fi
...
DEPLOYMENT_FILE="deployments/${CONTRACT_NAME}_${CHAIN_ID}_${FULL_SOURCE_VERSION}_prepare.txt"
