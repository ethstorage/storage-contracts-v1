...
compare_version() {
  ...
  echo ""
  echo "Current source code version: v$current_version-$GIT_COMMIT"
  echo "Deployed contract version:   v$deployed_version_full"
  ...
  case $result in
    0)
      echo "⚠️  Semantic versions are identical"
      return 0
      ;;
    1)
      return 1
...
# Check if current version with reference
...
NEW_IMPL_ADDRESS=$(grep -E "New implementation address: " "$OUTPUT_FILE" | tail -1 | awk '{print $NF}')
if [ -z "$NEW_IMPL_ADDRESS" ]; then
  echo "Error: New implementation address not found. Check $OUTPUT_FILE"
  exit 1
fi
...
BUILD_INFO_BACKUP_DIR="old-builds/build-info-$FULL_NEW_VERSION"
